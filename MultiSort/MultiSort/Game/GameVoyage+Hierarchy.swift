...
extension GameVoyage {
    ...
    func establishConstraints() {
        ...
        if variant.usesTopHintPlacement {
            mindfulHintConstraints = [
                hintButton.topAnchor.constraint(equalTo: scoreLabel.bottomAnchor, constant: 12),
                hintButton.trailingAnchor.constraint(equalTo: scoreLabel.trailingAnchor),
                hintButton.widthAnchor.constraint(equalToConstant: 120),
                hintButton.heightAnchor.constraint(equalToConstant: VibrantConstants.Measurements.buttonHeight * 0.8)
            ]
            NSLayoutConstraint.activate(mindfulHintConstraints)
        }
        
        if variant.usesSuitCycler {
            suitSwitcherConstraints = [
                suitSwitcherButton.centerYAnchor.constraint(equalTo: hintButton.centerYAnchor),
                suitSwitcherButton.leadingAnchor.constraint(equalTo: retreatButton.leadingAnchor),
                suitSwitcherButton.trailingAnchor.constraint(equalTo: hintButton.leadingAnchor, constant: -12),
                suitSwitcherButton.heightAnchor.constraint(equalTo: hintButton.heightAnchor)
            ]
